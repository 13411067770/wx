				...
				<view :id="o.key" v-for="(o,i) in resource" :key="i">
					...
					<view @click="clickFunc(item)" v-for="(item,index) in o.data" class="info" :key="index">
						<view :style="'background:'+color" class="icon">
							<image style="width: 100rpx; height: 100rpx; border-radius:25px;" :src="item.img" mode=""></image>
						</view>
						<view class="item">
							<text class="name">{{item[name]}}</text>
							<text class="post">{{item.companyName}}</text>
							<!-- <text v-if="item.createdAt" class="post">{{item.createdAt}}</text> -->
						</view>
					</view>
				</view>
			</scroll-view>
