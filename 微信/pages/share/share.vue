	...
	<view id="moments">
		...
		<view class="home-pic">
			<view class="home-pic-base">
				<view class="top-pic">
					<image class="header" src="../../static/image/logo4.jpg" @tap="test"></image>
				</view>
				<view class="top-name">lizj</view>
			</view>
		</view>

		<view class="moments__post" v-for="(post,index) in list" :key="index" :id="'post-'+index">
			<view class="post-left">
				<image class="post_header" :src="header_image"></image>
			</view>

			<view class="post_right">
				<text class="post-username">{{username}}</text>
				<view id="paragraph" class="paragraph">{{post.input_content}}</view>
				<!-- 相册 -->
				<!-- console.log({{post.imageList}}) -->
				<!-- {{post.imageList}} -->
				<view class="thumbnails">
					<view :class="post.imageList.imageList.length === 1?'my-gallery':'thumbnail'" v-for="(image, index_images) in post.imageList.imageList" :key="index_images">
						<image class="gallery_img" lazy-load mode="aspectFill" :src="image" :data-src="image" @tap="previewImage( post.imageList,index_images)"></image>
					</view>
				</view>
				<!-- 资料条 -->
				<view class="toolbar">
					...
					<view class="like" @tap="like(index)">
						<image :src="post.islike===0?'../../static/image/islike.png':'../../static/image/like.png'"></image>
					</view>
				...
				<!-- <view class="post-footer">
					<view class="footer_content">
						<image class="liked" src="../../static/image/liked.png"></image>
	...
	export default {
		...
		data() {
			return {
				...
				"header_image": "../../static/image/logo4.jpg",
				"username": "lizj",
				list:[
					{
						input_content: '',
		...
		onShow() {
			...
			uni.request({
			    ...
			    success: (res) => {
					...
					this.list = res.data
					// console.log(this.list.imageList)
					let obj = []
					for(let i of res.data){
					      i.imageList = JSON.parse(i.imageList)
						  // console.log("i.imageList",i.imageList)
					      obj.push(i)
					}
					// console.log("obj",obj.imageList)
					this.list = obj
			    }
			});
		},
